...
def get_default_file_and_hash():
    return 'setting/default.yml',\
        'ec154a74e9398712f318aa839206ea20'
...
@dataclasses.dataclass(frozen=True)
class Models:
    base_mlp: ModelsBaseMLP = dataclasses.field(init=False)

    def __post_init__(self):
        super().__setattr__('base_mlp', ModelsBaseMLP())
...
class Optimizer:
    ...
    def __post_init__(self):
        super().__setattr__('adam', OptimizerAdam())
...
class _Config:
    ...
    def __post_init__(self):
        super().__setattr__('models', Models())
        super().__setattr__('optimizer', Optimizer())
        super().__setattr__('training', Training())
        ...
        def __set(obj, d: typing.Dict[str, typing.Any]):
            for k, v in d.items():
                if not isinstance(v, dict):
                    ...
                    try:
                        __set(obj.__getattribute__(to_snake_case(k)), v)
...
def to_snake_case(name: str) -> str:
    return re.sub("($[A-Z]|[a-z][A-Z])", lambda c: c.group(1)[0] + "_" + c.group(1)[1], name).lower()
