...
class ConfigParser:
    ...
    def get_type(self, value: Any, this_class_name: Optional[str] = None, parent_class_name: Optional[str] = None) -> str:
        ...
        if isinstance(value, list) or isinstance(value, tuple):
            return f'Tuple[{", ".join([self.get_type(v, this_class_name) for v in value])}]'

        # Dict
        # 新たなNamedTupleとして新規クラス名を作成する
        # 作成の際は親要素のクラス名をprefixにし、重複を防ぐ
        if isinstance(value, dict):
            if parent_class_name is None:
                self.make_named_tuple_from_dict(this_class_name, value)
                parent_class_name = ''
    ...
    def make_named_tuple_from_dict(self, class_name: str, _dict: dict) -> None:
        # _dictのvalueにdictの型が存在しなくなるまで掘っていく
        for k, v in _dict.items():
            if isinstance(v, dict):
                self.make_named_tuple_from_dict(class_name + self.to_class_name(k), v)
            assert type(k) == str

        # _dictのkeyとvalueからNamedTupleを生成する
        class_def = "class {}(NamedTuple):\n    {}\n".format(
            class_name, '\n    '.join([self.parse(
                k, v, parent_class_name=class_name) for k, v in _dict.items()]))
        # 既に登録済みのclassはスルーする
        if class_name in self.named_tuples and len(self.named_tuples[class_name]) > len(class_def):
            return
        self.named_tuples[class_name] = class_def
